# explicit-repo-resolution Specification

## Purpose
specsync resolves the target repository explicitly and always passes `--repo` to every
`gh` invocation. Resolution order: explicit `-repo` flag → `gh repo set-default` →
`origin`. Fork-parent writes are refused by default. Board resolution is per-repository
only: `-project` flag → `openspec/specsync.yml` → no board; no global default.
## Requirements
