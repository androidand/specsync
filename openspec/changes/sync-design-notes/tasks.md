...
- [x] 1. Add `Change.DesignNotes` field; load `design.md` in `LoadChange`
...
- [x] 2. Render `## Design notes` in `WorkItemFor` (`sync.go`) when
...
- [x] 3. Decide and record the pull-side semantics (Open Question 1):
      ...
      Decision: write-once, like original-ask.md — see proposal.md's Open
      Questions.

- [x] 4. Decide comment-overflow default (Open Question 2): on by default or
      ...
      Decision: on by default, no flag — see proposal.md's Open Questions.

- [x] 5. Decide and implement the shrink-back-below-limit case (Open
...
- [x] 6. `-dry-run` previews the comment write the same way it already
      ...
      Decision: no dedicated flag needed — see proposal.md's Open Questions.

- [x] 7. New capability spec `openspec/specs/change-body-composition/spec.md`
      with Requirement/Scenario blocks covering: section set, ordering,
      omission when empty, and the overflow-to-comment behavior.

- [x] 8. Document in README's behaviors list (next to the existing
      Discoveries/Original ask bullets) and update `site/features.json` per
      AGENTS.md's dogfooding rule.

- [x] 9. Regression test end to end: a change with a large design.md that
