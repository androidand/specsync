...
- [ ] 1 Correct the archived `specsync-go-library` tasks and proposal to describe
...
- [ ] 2 State the canonical import path in package documentation, once.
  - File: `doc.go`
  - Validation: `doc.go` names `github.com/androidand/specsync` as the import
    path and no other path is documented anywhere
- [ ] 3 Add a guard test asserting the package location and the exported symbols
...
- [ ] 4 Link the downstream fix. skein's `specsync-library-integration` proposal
