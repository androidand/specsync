...
- `-close-completed` (default false): when every task in a change is checked, close the tracker item too. Without it, a completed change still updates the issue and moves it to the `stage:complete` label but leaves it open.
